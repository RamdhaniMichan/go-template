...
func (b *bookController) RetrieveDataBookInChannel(resp http.ResponseWriter, req *http.Request) {
	books, err := b.bookService.GetAllUsingChannel()
...
func (b *bookController) CreateBook(resp http.ResponseWriter, req *http.Request) {
	...
	authorID, _ := strconv.Atoi(req.FormValue("author_id"))
...
func (b *bookController) BulkInsert(resp http.ResponseWriter, req *http.Request) {
	...
	respBooks, errCreate := b.bookService.BulkInsert(books)
	...
	_ = function.SendResponse(resp, 201, "success", respBooks)
	return
}
